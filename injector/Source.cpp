...
DWORD RtlCreateUserThread(LPCSTR pszLibFile, DWORD dwProcessId)
{
	...
	RtlCreateUserThread = (pRtlCreateUserThread)GetProcAddress(GetModuleHandle("ntdll.dll"), "RtlCreateUserThread");
	...
	if (bStatus < 0)
	{
		printf("[-] Error: RtlCreateUserThread failed\n");
		return(1);
	}
	else
	{
		printf("[+] Remote thread has been created successfully ...\n");
...
int main()
{
	const char *name = "League of legends.exe";

	DWORD pId = findPidByName(name);
	LPCSTR location = "C:\\Users\\tonka\\source\\repos\\lol internal\\Debug\\lol internal.dll";
